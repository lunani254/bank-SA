...
const PaymentModal = ({ isOpen, onClose, onSubmit, title, isInternational = false }) => {
  ...
  const fetchRecipientName = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`https://localhost:3001/user/recipient/${paymentData.accountNumber}`);
      if (response.data.name) {
        setRecipientName(response.data.name);
        setShowConfirmation(true);
        setErrorMessage(''); // Clear error if successful
      } else {
        setErrorMessage('Recipient not found for the provided account number.');
      }
    } catch (error) {
      setErrorMessage('Error fetching recipient. Please check the account number and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSubmit(paymentData); // Pass payment data to the parent component's submit function
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input
              ...
              onBlur={fetchRecipientName} // Fetch recipient name when user leaves the account number field
            ...
            {showConfirmation ? (
              <>
                <button type="submit" className="submit-button" disabled={loading}>
                  {loading ? 'Processing...' : 'Confirm & Submit Payment'}
                </button>
                <button type="button" onClick={() => setShowConfirmation(false)} className="cancel-button">
                  Cancel
                </button>
              </>
            ) : (
              <button type="button" onClick={fetchRecipientName} className="submit-button">
                Fetch Recipient
              </button>
            )}
          </div>
...
const DashboardContent = ({ onPaymentSubmit }) => {
  ...
  const handlePaymentSubmit = async (paymentData, isInternational) => {
    try {
      ...
      // Debugging: Log the response data to check if it contains the expected success message
      console.log("Payment response:", response.data);
  
      if (response.data.message === "Payment processed successfully") {
        alert("Payment processed successfully"); // Notify the user of successful payment
      ...
      // Extract and display a meaningful error message to the user
